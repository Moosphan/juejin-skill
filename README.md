...
- 更新文章：`用 ./article.md 更新这篇掘金文章：https://juejin.cn/post/1234567890123456789`
- 下载文章：`下载这篇掘金文章：https://juejin.cn/post/1234567890123456789`

如果用户未指定分类，技能会先读取文章内容并推荐分类，再向用户确认后继续发布。

发布或更新 Markdown 时，形如 `![说明](./images/demo.png)` 的本地图片会先上传到掘金图片服务，再替换为线上图片地址；`http`、`https`、`data:` 等非本地图片地址会保持不变。更新文章默认只保存到草稿，如需更新线上文章，必须同时传入 `--publish --confirm-publish`。
